...
<head>
  ...
  <style>
    ...
    input, select {{ padding:8px; margin:6px 0; width:100%; max-width:500px }}
    button {{ padding:8px 16px; margin-top:12px }}
    .card {{ border:1px solid #eee; padding:20px; border-radius:6px; max-width:400px }}
  </style>
</head>
<body>
  <div class="nav">
...
def index():
    ...
    if request.method == "POST":
        ...
        if not nome or not sobrenome or not instituicao or not disciplina:
            return redirect(url_for("index"))  # força obrigatoriedade

        user_ip = request.remote_addr or "Desconhecido"
        host = request.host_url.rstrip("/")

        body = f"""
          <h1>Olá, {escape(nome)} {escape(sobrenome)}!</h1>
          <p>A sua Instituição de ensino é {escape(instituicao)}</p>
          <p>Está cursando a disciplina de {escape(disciplina)}</p>
          <p>O IP do computador remoto é: {escape(user_ip)}</p>
          <p>O host da aplicação é: {escape(host)}</p>
          <hr>
    ...
    body = f"""
      <h1>Olá, Estranho!</h1>
      <p>A sua Instituição de ensino é None</p>
      <p>Está cursando a disciplina de </p>
      <p>O IP do computador remoto é: None</p>
      <p>O host da aplicação é: None</p>
      <hr>
      <form method="post">
        ...
        <select name="disciplina" required>
          ...
          <option value="DWBA4">DWBA4</option>
